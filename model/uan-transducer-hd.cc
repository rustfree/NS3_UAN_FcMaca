                           ...
                           UanTxMode txMode)
{
  ...
  if (m_state == TX)
    {
      NS_LOG_DEBUG ("t-debug2");
      Simulator::Remove (m_endTxEvent);
      NS_LOG_DEBUG ("t-debug3");
  ...
  NS_LOG_DEBUG ("t-debug4 ,dataRateBps"<<txMode.GetDataRateBps ()<<" delay:"<<packet->GetSize () * 8.0 / txMode.GetDataRateBps ());
  Time delay = Seconds (packet->GetSize () * 8.0 / txMode.GetDataRateBps ());
  NS_LOG_DEBUG ("t-debug5");
